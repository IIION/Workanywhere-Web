<template>
  <div>
    <div class="dropdown">
      <div class="dropdown-title" @click="toggleMenu">
        {{ selectedTitle }} <span class="arrow-down"></span>
      </div>
      <ul v-if="showMenu" class="dropdown-list">
        <li
          :class="{ active: selectedTitle === '추천순' }"
          @click="selectOption('recommend')"
        >
          추천순
        </li>
        <li
          :class="{ active: selectedTitle === '출발일순' }"
          @click="selectOption('startdate')"
        >
          출발일순
        </li>
      </ul>
